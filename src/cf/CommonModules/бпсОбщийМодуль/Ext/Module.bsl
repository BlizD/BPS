Функция ПолучитьМассивРазрешенныхРеквизитов(ПредметСогласований)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	бпсПредметыСогласованийРазрешенныеРеквизиты.Реквизит КАК Реквизит
		|ИЗ
		|	Справочник.бпсПредметыСогласований.РазрешенныеРеквизиты КАК бпсПредметыСогласованийРазрешенныеРеквизиты
		|ГДЕ
		|	бпсПредметыСогласованийРазрешенныеРеквизиты.Ссылка = &ПредметСогласований";
	
	Запрос.УстановитьПараметр("ПредметСогласований", ПредметСогласований);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		РазрешенныеРеквизиты.Добавить(ВыборкаДетальныеЗаписи.Реквизит);
	КонецЦикла;
...
Функция ИзмененыТолькоРазрешенныеРеквизиты(Источник) Экспорт
	
	ТипОбъектаБД = Справочники.бпсТипыОбъектовБД.ПолучитьТипОбъектаБД(Источник.Ссылка);
	ПредметСогласований = Справочники.бпсПредметыСогласований.ПолучитьПредметСогласованияПоТипуОбъекта(ТипОбъектаБД);
	
	ОбъектМетаданные = Метаданные[ТипОбъектаБД.МенеджерОбъекта][ТипОбъектаБД.ОбъектБДИмяМетаданных];
	
	РазрешенныеРеквизиты = ПолучитьМассивРазрешенныхРеквизитов(ПредметСогласований);
	
	Если РазрешенныеРеквизиты.Количество() = 0 Тогда //если разрешенных реквизитов нет, дальнейшие проверки нет смысла выполнять
		Возврат Ложь;
	КонецЕсли; 
	...
	Для Каждого Реквизит Из Реквизиты Цикл
		...
		Если РазрешенныеРеквизиты.Найти(ИмяРек) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Возвр Тогда //проверяем дальше
	...
	Если Возвр Тогда
		...
		Для каждого ТЧ из ТабЧасти цикл
			...
			НомСтроки = 0;
			Пока НомСтроки < Источник[ТЧ.Имя].Количество() Цикл
				СтрокаТЧИсточник = Источник[ТЧ.Имя][НомСтроки];
				СтрокаТЧИсточникСсылка = Источник.Ссылка[ТЧ.Имя][НомСтроки];
				...
				Для Каждого Реквизит Из Реквизиты Цикл
					...
					Если СтрокаТЧИсточник[Реквизит.Имя] <> СтрокаТЧИсточникСсылка[Реквизит.Имя] Тогда
						Возвр = Ложь;
						Прервать;
					КонецЕсли;
				КонецЦикла;
...
Процедура бпсПередЗаписьюСправочниковПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Отказ и НЕ времОтказ Тогда //если отказ установлен именно из-за блокировки объекта, проверим, может изменились только разрешенные реквизиты
		Отказ = НЕ ИзмененыТолькоРазрешенныеРеквизиты(Источник);
		Если Отказ Тогда
			...
			Иначе
				...
				Сообщение.Текст = "Ошибка! запрещено изменять объект согласования ["+Источник.Ссылка+"]";
				Сообщение.Сообщить();
			Конецесли;
		КонецЕсли;
	КонецЕсли;
